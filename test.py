...
while True:
    for series in dfS:
        try:
            ...
            dfS["key"] = False if len(orders) >= 1 or dfS["count"] < 3001 else True
            df = dfS["df"]
            histogram_5 = dfS["histogram_5"]
        # Check for duplicate time and remove corresponding elements from lists
            ...
            trigger = get_95(histogram_5)
            # Place sell order if MACD crosses above a threshold
            if value >= trigger and dfS["key"]:
